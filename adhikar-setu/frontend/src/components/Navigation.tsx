        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-2">
              {/* Notifications */}
              <button
                className={`p-2 rounded-lg ${isDarkMode ? 'hover:bg-gray-800 text-gray-300' : 'hover:bg-gray-100 text-gray-600'} transition-colors duration-200 relative`}
              ...
              <button
                ...
                className={`p-2 rounded-lg ${isDarkMode ? 'hover:bg-gray-800' : 'hover:bg-gray-100'} transition-colors duration-200`}
                title={isDarkMode ? (language === 'en' ? 'Light mode' : 'लाइट मोड') : (language === 'en' ? 'Dark mode' : 'डार्क मोड')}
              >
              ...
              <div className="relative hidden md:block">
                <button
                  ...
                  className={`flex items-center space-x-2 p-2 rounded-lg ${isDarkMode ? 'hover:bg-gray-800' : 'hover:bg-gray-100'} transition-colors duration-200`}
                >
                  ...
                  <ChevronDown className={`h-4 w-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} transition-transform duration-200 ${isProfileDropdownOpen ? 'rotate-180' : ''}`} />
                </button>

                {/* Dropdown Menu */}
                {isProfileDropdownOpen && (
                  <div className={`absolute right-0 mt-2 w-48 rounded-lg shadow-lg py-1 ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'} z-50 transition-all duration-200`}>
                    <button
                      className={`flex items-center w-full px-4 py-2 text-sm ${isDarkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-100'} transition-colors duration-150`}
                    >
                    ...
                    <button
                      onClick={onLogout}
                      className={`flex items-center w-full px-4 py-2 text-sm ${isDarkMode ? 'text-red-400 hover:bg-gray-700' : 'text-red-600 hover:bg-gray-100'} transition-colors duration-150`}
                    >
              ...
              <div className="md:hidden">
                <button
                  ...
                  className={`p-2 rounded-lg ${isDarkMode ? 'hover:bg-gray-800 text-white' : 'hover:bg-gray-100 text-gray-600'} transition-colors duration-200`}
                >
          ...
          {isMenuOpen && (
            <div className="md:hidden">
              <div className={`px-2 pt-2 pb-3 space-y-1 sm:px-3 border-t ${isDarkMode ? 'border-gray-800' : 'border-gray-200'} transition-colors duration-300`}>
                {menuItems.map((item) => {
                  const Icon = item.icon;
                  return (
                    <button
                ...
                <div className="pt-4 pb-3 border-t border-gray-200 dark:border-gray-800">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3">
                    <button
                      className={`flex items-center justify-center px-3 py-2 rounded-lg text-sm font-medium ${isDarkMode ? 'bg-gray-800 text-gray-300 hover:bg-gray-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'} transition-colors duration-200`}
                    >
