...
const LocationSelection: React.FC<LocationSelectionProps> = ({ formData, updateFormData }) => {
    ...
    const getDistricts = (stateId: string): string[] => {
        const districtData: Record<string, string[]> = {
            ...
            tg: ['Adilabad', 'Bhadradri', 'Jayashankar', 'Komaram Bheem', 'Mancherial']
        };
        return districtData[stateId] || [];
    };

    const getVillages = (districtName: string): string[] => {
        // Mock villages - replace with Firestore query
        const villageData: Record<string, string[]> = {
    ...
    const needsDistrict = !['slmc', 'mota', 'ngo', 'researcher', 'public'].includes(role);
    const needsState = !['slmc', 'mota', 'ngo', 'researcher', 'public'].includes(role);

    const getLocationNote = (): string => {
        if (needsVillage) {
            return language === 'hi'
        ...
        <div>
            <div className="text-center mb-6">
                ...
                <p className="text-gray-600 text-sm">
                    {language === 'hi' ? 'कृपया अपनी कार्य स्थिति बताएं' : 'Please provide your work location'}
                </p>
            </div>
                        ...
                        <div className="space-y-2">
                            {states.map((stateOption) => (
                                <button
                                    ...
                                    className={`w-full p-3 border-2 rounded-lg text-left transition-all duration-200 ${state === stateOption.id
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div
                                            className={`w-4 h-4 rounded-full border-2 ${state === stateOption.id
                        ...
                        <div className="space-y-2">
                            {getDistricts(state).map((districtOption) => (
                                <button
                                    ...
                                    className={`w-full p-3 border-2 rounded-lg text-left transition-all duration-200 ${district === districtOption
                                        ? 'border-green-600 bg-green-50'
                                        : 'border-gray-200 hover:border-green-300 hover:bg-gray-50'
                                        }`}
                                >
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div
                                            className={`w-4 h-4 rounded-full border-2 ${district === districtOption
                                                ? 'border-green-600 bg-green-600'
                                                : 'border-gray-300'
                                                }`}
                                        >
                        ...
                        <div className="space-y-2">
                            {getVillages(district).map((villageOption) => (
                                <button
                                    ...
                                    className={`w-full p-3 border-2 rounded-lg text-left transition-all duration-200 ${village === villageOption
                                        ? 'border-green-600 bg-green-50'
                                        : 'border-gray-200 hover:border-green-300 hover:bg-gray-50'
                                        }`}
                                >
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div
                                            className={`w-4 h-4 rounded-full border-2 ${village === villageOption
                                                ? 'border-green-600 bg-green-600'
                                                : 'border-gray-300'
                                                }`}
                                        >
